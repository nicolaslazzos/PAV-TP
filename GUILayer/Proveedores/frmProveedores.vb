Public Class frmProveedores
    ...
    Private Sub frmProveedores_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        tabProveedores.SelectedTab = tabNuevo

        tabProveedores.TabPages("tabModificar").Enabled = False
    ...
    Private Sub llenarGrid(ByVal source As DataTable)
        ...
        For Each fila As DataRow In source.Rows
            dgvProveedores.Rows.Add(New String() {fila.Item("CUIT").ToString, fila.Item("razonSocial").ToString, fila.Item("telefono").ToString, fila.Item("Provincia").ToString, fila.Item("Localidad").ToString, fila.Item("Barrio").ToString, fila.Item("calle").ToString, fila.Item("numero").ToString, fila.Item("codProvincia").ToString, fila.Item("codLocalidad").ToString, fila.Item("codBarrio").ToString})
        Next
    End Sub

    Private Sub llenarGrid()
        Dim str As String = "SELECT p.CUIT, p.razonSocial, p.telefono, p.calle, p.numero, b.nombre AS Barrio, l.nombre AS Localidad, pro.nombre AS Provincia, pro.codProvincia, l.codLocalidad, b.codBarrio"
        str += " FROM Proveedores p INNER JOIN Barrios b ON (p.codBarrio = b.codBarrio) INNER JOIN Localidades l ON (l.codLocalidad = b.codLocalidad) INNER JOIN Provincias pro ON (pro.codProvincia = l.codProvincia) WHERE p.bajaLogica='False'"
    ...
    Private Sub dgvProveedores_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvProveedores.CellContentClick
        ...
        rowSelected = dgvProveedores.CurrentRow
    End Sub
    Private Sub cmdNuevo_Click(sender As Object, e As EventArgs) Handles cmdNuevo.Click
        Dim validar As DataTable
        Dim filters As New List(Of Object)
        Dim str As String = "INSERT INTO Proveedores (CUIT,razonSocial,telefono,calle,numero,codBarrio,fechaAlta) VALUES (@param1,@param2,@param3,@param4,@param5,@param6,@param7)"
        ...
        validar = BDHelper.getDBHelper.ConsultaSQL("SELECT CUIT FROM Proveedores WHERE CUIT=" & txtCUIT.Text)
        If validar.Rows.Count > 0 Then
            Mensajes.mostrarError("Ya existe un proveedor con el CUIT ingresado!")
        Else
    ...
    Private Sub cmdConsultar_Click(sender As Object, e As EventArgs) Handles cmdConsultar.Click
        ...
        If cmbProvincia.Text <> String.Empty Then
            ...
            str += " AND pro.codProvincia=@param4"
        Else
            filters.Add(Nothing)
        End If
    ...
    Private Sub cmdEliminar_Click(sender As Object, e As EventArgs) Handles cmdEliminar.Click
        Dim filters As New List(Of Object)
        Dim Str As String = "UPDATE Proveedores SET bajaLogica='True' WHERE CUIT=@param1"
    ...
    Private Sub cmdVerUbicacion_Click(sender As Object, e As EventArgs) Handles cmdVerUbicacion.Click
        Dim selectedRow As DataGridViewRow = dgvProveedores.CurrentRow
    ...
    Private Sub cmdModificar_Click(sender As Object, e As EventArgs) Handles cmdModificar.Click
        tabProveedores.SelectedTab = tabModificar
        tabProveedores.TabPages("tabModificar").Enabled = True
    ...
    Private Sub cmdGuardar_Click(sender As Object, e As EventArgs) Handles cmdGuardar.Click
        Dim filters As New List(Of Object)
        Dim str As String = "UPDATE Proveedores SET "
